...
begin
  ...
  if v_items <> coalesce(array_length(p_allocation_amounts, 1), 0) then
    raise exception 'Allocation arrays length mismatch';
  end if;

  if p_installments_count < 1 or p_installments_count > 120 then
  ...
  values (v_user_id, p_name, round(p_total_amount::numeric, 2), p_purchase_date, p_installments_count)
  ...
  for v_index in 1..v_items loop
    ...
    values (v_allocation_id, v_user_id, p_installments_count, 'monthly', p_purchase_date)
    returning id into v_plan_id;

    v_base_installment := floor((v_amount * 100) / p_installments_count) / 100;
    v_last_adjustment := v_amount - round(v_base_installment * p_installments_count, 2);
    ...
    select
      ...
      case
        when installment_idx = p_installments_count then round(v_base_installment + v_last_adjustment, 2)
        else round(v_base_installment, 2)
      end,
      'pending'
    from generate_series(1, p_installments_count) as installment_idx;
  end loop;
